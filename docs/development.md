...
| `server.js` · `server/work.js` · `server/chats.js` · `server/store.js` · `server/files.js` · `server/sandbox.js` · `server/mcp/` · `server/env/` | 本机桥接：转发、联网、静态与拼接；执事接口（目录、指令、文件、检索、目录选择框、后台指令）、卷宗目录接口（列、收、取、删）、对话目录接口（整读、存、删）、附件目录接口（存、取、删、清）与存储目录（配置的读写、旧数据迁入、换位置）；MCP 服务池（起、连外部服务，列工具、调工具）；沙箱环境（装、查、清，给桥接起的进程接上）；沙箱的三道筛（指令、路径、环境变量）是纯函数，单独一段 |
