...
(window.YAN_PROMPTS ||= {}).tools = {
  ...
  delegate: {
    description:
      "差遣一名帮手独立完成一件自成一段的子任务（通读一批文件并归纳、在某个模块里排查一个问题、按已定方案实现独立的一部分），做完回报给你。帮手有与你相同的工具与工作目录，但看不到这段对话：task 里要写全背景、目标、边界与要回报的内容。适合量大、独立、或会读入大量与主线无关内容的活；一两步的事直接做。同一轮里可差遣多名帮手并行，分派时让他们改的文件互不重叠。",
    brief:
